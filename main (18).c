...
int main(int argc, char *argv[]) {
  ...
  printResult(fullyA,3);  // value incorrect 
  printResultfor2Set(twoWay);
  // print out the cache statistics for each of the three cache types
  free(directA);
  for(int i = 0; i < 2; i++)
  {
    free(twoWay[i]);
...
struct cache * directMapped(char * address, struct cache *addresses)
{
  //// Convert the hexadecimal address to decimal and get the block number in the cache.
  ...
  return addresses; 
  // // Return the updated cache structure.
}
...
//the loop is broken, and the updated cache addresses are returned.
struct cache ** twoWayAssociative(char * address, struct cache **addresses, int rp)
// takes in an address to access
{
  int ret = hextoDecimal(address);
...
//If the address is already in the cache, the corresponding cache block's hit count is incremented.
void printResult (struct cache *addresses, int assoc)
{
  ...
  //// Loop through each block in the cache
  for(int i = 0; i < 32; i++ )
  {
  ...
  //// If the associative mapping is fully associative, print the appropriate message
  printf("Total Accesses: %d\n", total_cache_accesses);
  printf("Total cache_hits: %d\n", total_hit);
  printf("Hit rate: %f\n\n", (float)total_hit/total_cache_accesses);
  //// Print the total number of cache accesses
}
...
void printResultfor2Set(struct cache **addresses)
{
  ...
  printf("Total cache_hits: %d\n", total_hit);
  printf("Hit rate: %f\n\n", (float)total_hit/total_cache_accesses);
  // Print the total number of cache accesses, total cache hits, and the hit rate
}